const CACHE_NAME='cysh-manager-v10.0-shell';
...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'||url.pathname.endsWith('/data/opportunities.json')){
...
self.addEventListener('notificationclick',event=>{
  ...
  event.waitUntil(self.clients.matchAll({type:'window',includeUncontrolled:true}).then(async clients=>{
    for(const client of clients){
      if('focus' in client){await client.focus();client.postMessage({type:'NAVIGATE',page});return;}
    }
...
self.addEventListener('push',event=>{
  ...
  const options={body:data.body||'有新的提醒。',icon:'./icons/icon-192.png',badge:'./icons/icon-192.png',tag:data.tag||'cysh-push',data:{page:data.page||'home'}};
